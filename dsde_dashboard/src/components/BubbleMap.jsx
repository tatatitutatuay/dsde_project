...
const BubbleMap = ({ populationData, worldMap }) => {
    ...
    useEffect(() => {
        ...
        const data = populationData
            .map((d) => ({
                ...
                country: worldMap.features.find(
                    (feature) =>
                        feature.properties.ISO_A3 === d.iso_a3 &&
                        d.iso_a3 !== 'ATA' // Exclude Antarctica (ISO_A3: ATA)
                ),
            }))
        ...
        svg.append('g')
            .selectAll('path')
            .data(
                worldMap.features.filter(
                    (feature) => feature.properties.ISO_A3 !== 'ATA'
                ) // Exclude Antarctica
            )
            .join('path')
            .attr('fill', 'none')
        ...
        // Draw circles for each country (up to 5 circles per country)
        ...
        svg.append('g')
            ...
            .data(
                data.flatMap((d) => {
                    // Ensure that 5 circles are created for each country
                    const additionalCircles = 5 - d.keyword.length;
                    const newKeywords = [
                        ...d.keyword,
                        ...Array(additionalCircles).fill({
                            name: 'No data',
                            frequency: 0,
                        }),
                    ];

                    return newKeywords.map((k, idx) => ({
                        country: d.country,
                        keyword: k,
                        iso_a3: d.iso_a3,
            ...
            .attr('transform', (d) => {
                ...
                // Offsets for positioning the circles (center + offsets)
                const offsets = [
                    [0, 0], // Center
                    [-5, 0], // Left
                    [5, 0], // Right
                ...
                const offset = offsets[d.offset] || [0, 0];
        ...
        const legend = svg
            ...
            .selectAll('g')
            .data([0, 1, 2, 3, 4]) // Data for the five keywords
            .join('g');
